    ...
    FV: FnMut(&N, &N) -> Option<C>,
{
    ...
    while let Some(SmallestCostHolder { cost, index, .. }) = to_see.pop() {
        ...
        for (successor, move_cost) in successors {
            let (index, cost, move_cost) = parents
                .get_index(parent_index)
                .map(|(parent, (_, parent_cost))| {
                    sight(&successor, parent)
                        .map(|parent_move_cost| (parent_index, *parent_cost, parent_move_cost))
                })
                .flatten()
                .unwrap_or((index, cost, move_cost));
